...
type TextImageProps = Partial<HeadlineProps> &
  ContainerProps & {
...
export default function TextImage(props: TextImageProps) {
  ...
  const textBody = body && body.value && (
    <div
      className={`text-font text-xl leading-8 ${image ? `mt-4 md:mt-0 md:w-1/2 ${imagePositionedRight ? 'md:mr-10' : 'md:ml-10'}` : ''}`}
    >
      {title && title.value && (
        <Headline headline={headline || title.value} {...rest} />
      )}
    ...
    >
      <GridContainer>
        <div
          className={
            image
              ? 'col-span-6 md:col-span-12 md:col-start-2'
              : 'col-span-6 md:col-span-7 md:col-start-3'
          }
        >
